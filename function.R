...
var_select_backward<-function(datax, event, method, number=10, repeats=1, var_drop_rate=0.05,
                              mtry_form=NULL, nroundx=200, c.sd=1, rcv_ce=FALSE, nthreadx=1,
                              cf_threshold=NULL, ntreex=NULL, minc_x=NULL){
  datax1<-datax
  ...
  if(method=="cforest"){
    ...
    #cf0<-cforest(event~., data=datax1, control=ctree_control(MIA = TRUE, mincriterion = minc_x), 
    #             ntree=ntreex, mtry = mtryx)
    ...
    while(nvar>=1){ 
      ...
      if(i==1){
        ...
        #cf1<-cforest(event~., data=datax1, control=ctree_control(MIA = TRUE, mincriterion = minc_x), 
        #             ntree=ntreex, mtry = mtryx)
        cf1<-cforest(event~., data=datax1, control=ctree_control(MIA = TRUE))
      }
