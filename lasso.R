...
impute_boot<-foreach(i=1:n_rep, .combine="comb", .multicombine = TRUE,  .init=list(list()), 
              ...
              foreach(b=1:n_boot_impute, .combine="comb", .multicombine = TRUE, .init=list(list()),
                      .packages = c("doParallel")) %dopar%{
                        ...
                        # Model
                        cov_mat<-model.matrix(~., mice_b_dat[,-which(colnames(mice_b_dat)%in%c("y"))])[,-1] # Remove intercept
                        out_vec<-ifelse(mice_b_dat$y=="Yes",1,
                                        ifelse(mice_b_dat$y=="No",-1,NA))
                        
                        # Find the best lambda using cross-validation
                        cv.lasso<-cv.glmnet(x=cov_mat, y=out_vec, family="binomial", nfolds = 10, alpha=1)
                        
                        # Fit the final model on the training data (using minimum Lambda)
                        m1<-glmnet(x=cov_mat, y=out_vec, lambda=cv.lasso$lambda.1se, family="binomial", alpha=1)
                        
                        final_var_b<-rownames(coef(m1))[as.numeric(as.character(coef(m1)))!=0 & rownames(coef(m1))!="(Intercept)"]
                        final_var_b2<-ifelse(final_var_b%in%c("x11"),"x1",
                                             ifelse(final_var_b%in%c("x21"),"x2", 
                                                    ifelse(final_var_b%in%c("z211"),"z21", 
                                                           ifelse(final_var_b%in%c("z221"),"z22", 
                                                                  ifelse(final_var_b%in%c("z231"),"z23", 
                                                                         ifelse(final_var_b%in%c("z241"),"z24", 
                                                                                ifelse(final_var_b%in%c("z251"),"z25", 
                                                                                       ifelse(final_var_b%in%c("z261"),"z26", 
                                                                                              ifelse(final_var_b%in%c("z271"),"z27", 
                                                                                                     ifelse(final_var_b%in%c("z281"),"z28", 
                                                                                                            ifelse(final_var_b%in%c("z291"),"z29", 
                                                                                                                   ifelse(final_var_b%in%c("z301"),"z30",
                                                                                                                          ifelse(final_var_b%in%c("z311"),"z31", 
                                                                                                                                 ifelse(final_var_b%in%c("z321"),"z32", 
                                                                                                                                        ifelse(final_var_b%in%c("z331"),"z33", 
                                                                                                                                               ifelse(final_var_b%in%c("z341"),"z34", 
                                                                                                                                                      ifelse(final_var_b%in%c("z351"),"z35", 
                                                                                                                                                             ifelse(final_var_b%in%c("z361"),"z36", 
                                                                                                                                                                    ifelse(final_var_b%in%c("z371"),"z37", 
                                                                                                                                                                           ifelse(final_var_b%in%c("z381"),"z38", 
                                                                                                                                                                                  ifelse(final_var_b%in%c("z391"),"z39", 
                                                                                                                                                                                         ifelse(final_var_b%in%c("z401"),"z40", as.character(final_var_b) 
                                                                                                                                                                                         )) ))))) ))))) ))))) )))))
                        selected_var_b<-unique(final_var_b2)
                        list("selected_var_b"=selected_var_b)
                      }
stopCluster(cl)
