...
source("/Volumes/GoogleDrive/My Drive/SWAN Study/Codes/function.R")
...
impute_boot<-foreach(i=1:n_rep, .combine="comb", .multicombine = TRUE,  .init=list(list()), 
                       ...
                       foreach(b=1:n_boot_impute, .combine="comb", .multicombine = TRUE, .init=list(list()),
                               .packages = c("doParallel")) %dopar%{
                                 ...
                                 out_vec<-factor(ifelse(mice_b_dat$y=="Yes",1,
                                                        ifelse(mice_b_dat$y=="No", 0,NA)))
                                 rf_varselect<-varSelRF(cov_mat, out_vec, vars.drop.frac = vars.drop.frac_x)
                                 
                                 selected_var_b<-rf_varselect$selected.vars
